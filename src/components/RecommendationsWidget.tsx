  ...
  const fetchRecommendations = async () => {
    try {
      ...
      if (data.length === 0) {
        ...
        const prompt = `Generate 3 smart farming recommendations for today. 
Return ONLY a valid JSON array of objects, with NO markdown formatting, NO \`\`\`json block, just the raw array. 
Each object must have these exact keys:
- title (string): short action-oriented title
- description (string): detailed explanation
- priority (string): exactly 'high', 'medium', or 'low'
- category (string): e.g., 'Weather', 'Crop Health', 'Irrigation'`;

        try {
          const aiResponse = await generateGeminiResponse(prompt);
          // Clean up the response in case the AI added markdown blocks anyway
          const cleanJson = aiResponse.replace(/```json/g, '').replace(/```/g, '').trim();
          const generatedRecs = JSON.parse(cleanJson);
          
          const savedRecs = [];
          for (const rec of generatedRecs) {
            const saved = await RecommendationsAPI.createRecommendation({
              ...rec,
              status: 'active',
              metadata: {}
            });
          ...
          // Just fail silently for generation, will show empty state
          setRecommendations([]);
        }
      } else {
